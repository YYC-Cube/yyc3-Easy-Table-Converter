...
exports.createConversionService = createConversionService;
const logger_1 = require("../utils/logger");
/**
 * 转换服务类
 */
class ConversionService {
    constructor(storageService, queueManager) {
        this.tasks = new Map();
     ...
     * @param job 任务对象
     */
    async processConversionTask(job) {
        ...
        try {
            ...
            logger_1.logger.error(`❌ 任务处理失败: ${taskId}`, error);
            // 更新任务状态为失败
            const updatedTask = this.tasks.get(taskId);
            if (updatedTask) {
     ...
     */
    async convertData(inputFile, outputFormat, options = {}) {
        try {
            logger_1.logger.info(`🔄 开始转换文件: ${inputFile.name} (${inputFile.format} -> ${outputFormat})`, {
                inputFile,
                outputFormat,
                options
            });
            // 验证输入格式是否支持
            if (!this.supportedFormats.includes(inputFile.format)) {
            ...
            // 根据格式组合选择转换策略
            const conversionStrategy = this.getConversionStrategy(inputFile.format, outputFormat);
            // 执行转换
            ...
            logger_1.logger.info(`✅ 转换完成: ${outputFile.name}`, {
                outputFile,
            ...
            logger_1.logger.error('❌ 转换失败:', error);
            return {
                outputFile: {},
                status: 'failed',
     ...
     * @returns 转换函数
     */
    getConversionStrategy(inputFormat, outputFormat) {
        // 简单实现 - 实际项目中需要根据不同格式组合调用相应的转换器
        return async (inputFile, options) => {
            // 读取文件内容
     ...
     * @param options 转换选项
     */
    convertToJSON(content, inputFormat, options) {
        // 简单实现 - 实际项目中需要根据不同格式实现转换逻辑
        logger_1.logger.debug(`🔄 转换为JSON: ${inputFormat} -> json`);
     ...
     * @param options 转换选项
     */
    convertToCSV(content, inputFormat, options) {
        // 简单实现 - 实际项目中需要根据不同格式实现转换逻辑
        logger_1.logger.debug(`🔄 转换为CSV: ${inputFormat} -> csv`);
     ...
     * @param options 转换选项
     */
    convertToXML(content, inputFormat, options) {
        // 简单实现 - 实际项目中需要根据不同格式实现转换逻辑
        logger_1.logger.debug(`🔄 转换为XML: ${inputFormat} -> xml`);
     ...
     * @param options 转换选项
     */
    convertToYAML(content, inputFormat, options) {
        // 简单实现 - 实际项目中需要根据不同格式实现转换逻辑
        logger_1.logger.debug(`🔄 转换为YAML: ${inputFormat} -> yaml`);
        return `message: 转换结果\ncontent: ${content}\nformat: ${inputFormat}`;
    }
    /**
     * 转换为TOML格式
     ...
     * @param options 转换选项
     */
    convertToTOML(content, inputFormat, options) {
        // 简单实现 - 实际项目中需要根据不同格式实现转换逻辑
        logger_1.logger.debug(`🔄 转换为TOML: ${inputFormat} -> toml`);
        return `message = "转换结果"\ncontent = "${content}"\nformat = "${inputFormat}"`;
    }
    /**
     * 转换为HTML格式
     ...
     * @param options 转换选项
     */
    convertToHTML(content, inputFormat, options) {
        // 简单实现 - 实际项目中需要根据不同格式实现转换逻辑
        logger_1.logger.debug(`🔄 转换为HTML: ${inputFormat} -> html`);
     ...
     * @param options 转换选项
     */
    convertToMarkdown(content, inputFormat, options) {
        // 简单实现 - 实际项目中需要根据不同格式实现转换逻辑
        logger_1.logger.debug(`🔄 转换为Markdown: ${inputFormat} -> markdown`);
        return `# 转换结果\n\n**内容**: ${content}\n\n**格式**: ${inputFormat}`;
    }
    /**
     * 计算记录数量
     ...
     */
    async createConversionTask(inputFile, outputFormat, options = {}, userId) {
        try {
            // 生成唯一任务ID
            ...
            logger_1.logger.info(`📥 转换任务创建成功: ${taskId}`, {
            ...
            logger_1.logger.error('❌ 转换任务创建失败:', error);
            throw error;
        }
    }
    /**
     ...
     * @param taskId 任务ID
     */
    async getConversionTask(taskId) {
     ...
     * @param updates 更新内容
     */
    async updateConversionTask(taskId, updates) {
        const task = this.tasks.get(taskId);
        if (!task) {
            return null;
        }
     ...
     * @param taskId 任务ID
     */
    async cancelConversionTask(taskId) {
        try {
            const task = this.tasks.get(taskId);
            if (!task) {
                return false;
            ...
            logger_1.logger.info(`⏹️  转换任务已取消: ${taskId}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error('❌ 取消转换任务失败:', error);
            return false;
        }
    }
    /**
     ...
     * @param userId 用户ID
     */
    async batchCreateConversionTasks(tasks, userId) {
        try {
            const taskIds = [];
            for (const taskData of tasks) {
                const taskId = await this.createConversionTask(taskData.inputFile, taskData.outputFormat, taskData.options || {}, userId);
                taskIds.push(taskId);
            }
            logger_1.logger.info(`📥 批量转换任务创建成功，共 ${taskIds.length} 个任务`);
            return taskIds;
        }
        catch (error) {
            logger_1.logger.error('❌ 批量创建转换任务失败:', error);
            throw error;
        }
    }
}
 ...
 * 导出服务实例创建函数
 */
function createConversionService(storageService, queueManager) {
