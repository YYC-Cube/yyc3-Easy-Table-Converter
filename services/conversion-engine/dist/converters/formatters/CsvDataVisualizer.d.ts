 ...
 * 图表类型枚举
 */
export type ChartType = 'bar' | 'line' | 'pie' | 'scatter' | 'histogram' | 'box' | 'area' | 'radar' | 'heatmap';
/**
 * 图表主题枚举
 */
export type ChartTheme = 'light' | 'dark' | 'default' | 'pastel' | 'vintage' | 'modern';
/**
 * 坐标轴配置接口
 */
 ...
 * CSV数据可视化选项接口
 */
export interface CsvDataVisualizerOptions extends ConversionOptions {
    chartType: ChartType;
 ...
 * CSV数据可视化工具类
 */
export declare class CsvDataVisualizer extends BaseConverter {
    constructor();
    /**
     * 执行CSV数据可视化
     ...
     * @param options 可视化选项
     */
    convert(inputData: Buffer | string, inputFormat: string, outputFormat: string, options: CsvDataVisualizerOptions): Promise<ConversionResult>;
    /**
     * 创建可视化
     * @param inputData CSV输入数据
     * @param outputFormat 输出格式
...
//# sourceMappingURL=CsvDataVisualizer.d.ts.map