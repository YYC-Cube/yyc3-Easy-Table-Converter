 ...
 * @module __tests__/encrypt/encryptDecrypt.test
 * @author YYC
 * @version 1.0.0
 * @created 2026-05-24
 */
...
describe('AES Encryption Module', () => {
  ...
  describe('decrypt', () => {
    ...
    it('should handle invalid JSON format', async () => {
      await expect(decrypt('not-valid-json', 'password')).rejects.toThrow('无效的加密数据格式');
    });

    it('should handle incomplete encrypted data', async () => {
      const incompleteData = JSON.stringify({ salt: 'abc' });
      
      await expect(decrypt(incompleteData, 'password')).rejects.toThrow('加密数据结构不完整');
    });
